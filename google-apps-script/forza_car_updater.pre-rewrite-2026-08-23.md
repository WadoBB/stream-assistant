...
function diagnoseMatching() {
  ...
  SpreadsheetApp.getActive().toast(
    ...
    `${results.excludedByNotes || 0} Results rows excluded by Notes ` +
    `(Spec/Touge/Time Attack - never counted as MATCH or MISS). ` +
...
function updateForzaCars_() {
  ...
  const summary = {
    ...
    excludedByNotes: results.excludedByNotes || 0,
...
/**
 * Loads the Results tab.  Returns array of normalised result rows, with an
 * extra `excludedByNotes` property (count of rows skipped below) attached
 * directly to the array so callers can surface it without a signature change.
 ...
 * so including them would inflate win rates. This is the single most common
 * reason a car with real Results rows shows 0 Races/Wins on the Cars tab —
 * check Notes before suspecting a matching bug.
 */
function readResults_(ss) {
  ...
  const values = sheet.getDataRange().getValues();
  if (values.length < 2) { const empty = []; empty.excludedByNotes = 0; return empty; }
  ...
  for (let i = 1; i < values.length; i++) {
    ...
    if (notes === 'Spec Race' || notes === 'Touge' || notes === 'Touge Race' || notes === 'Time Attack') {
      excludedByNotes++;
      continue;
    }
  ...
  out.excludedByNotes = excludedByNotes;
  return out;
}
...
function applyUpdates_(ss, cars, stats, yWinners) {
  ...
  // resolveCol_ double-checks each name actually still lives at the index the
  // header map claims, and throws immediately (naming the exact mismatch) if
  ...
  for (const car of cars.rows) {
    ...
    if (s) {
      ...
      // they can never disagree (previously only Races was zeroed here,
      // leaving Wins stranded at a stale value - see COMPUTED_COLS comment).
      winR[idx][0]  = '';
      races[idx][0] = 0;
      wins[idx][0]  = 0;
      bestT[idx][0] = '';
...
/** Converts a 1-based column number to its spreadsheet letter (1 -> A, 14 -> N, 27 -> AA). */
